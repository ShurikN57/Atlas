...
async function runCase(test) {
  ...
  let parcel = null;
  try {
    const parcelUrl = `https://data.geopf.fr/geocodage/reverse?lon=${encodeURIComponent(lon)}&lat=${encodeURIComponent(lat)}&index=parcel&limit=1`;
    const parcelData = await json(parcelUrl);
    const pp = parcelData.features?.[0]?.properties || {};
    parcel = pp.id || pp.parcel_id || pp.parcelle || pp.name || pp.label || null;
  } catch (error) {
    parcel = `indisponible (${error.message})`;
  }
